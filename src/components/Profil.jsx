...
export default function Profil({ me, onChange, onSave, isAnonymous, onLogin }) {
  const field =
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 py-10">
      ...
      {isAnonymous && (
        <div className="mt-6 flex items-center justify-between gap-4 rounded-2xl border border-pasar-ochre/30 bg-pasar-ochre/5 p-4">
          <p className="font-body text-sm text-pasar-ink/70">
            Masuk pakai Google supaya namamu tampil di header, dan profil ini
            tetap tersimpan seperti sekarang.
          </p>
          <button
            onClick={onLogin}
            className="shrink-0 rounded-full bg-pasar-ochre px-4 py-2 font-body text-sm font-semibold text-white hover:scale-[1.02] transition-transform"
          >
            Masuk dengan Google
          </button>
        </div>
      ...
      <div className="mt-8 space-y-5 rounded-2xl border border-pasar-ink/10 bg-white p-6">
        ...
        <button
          onClick={onSave}
          className="w-full rounded-full bg-pasar-ochre px-6 py-3 font-body text-sm font-semibold text-white transition-transform hover:scale-[1.01] active:scale-[0.99]"
        >
          Simpan profil
        </button>
      </div>
